...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  // Glamsphere pattern (proven to work in production)
  // Smart image detection for ALL content types
  let imageUrl: string | undefined
  
  // Type-safe check: use 'as any' for runtime data that might not be in compile-time types
  const postData = post as any
  if (postData.contentType === 'affiliate-product-review') {
    // Use Amazon image for affiliate products
    imageUrl = postData.typeSpecificData?.affiliateProductReview?.affiliate?.images?.[0]
  } else if (post.image && post.image !== '/placeholder.svg') {
    // Use hero/featured image for other content types (if not placeholder)
    imageUrl = post.image
  }
  // else: leave undefined - no image metadata (safe)
  ...
  return {
    ...
    twitter: {
      ...
      title: post.title,
      description: post.excerpt,
      ...(imageUrl && { images: [imageUrl] }),  // Only add if imageUrl exists
    },
  }
}
